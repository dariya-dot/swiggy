...
const venderRegister = async (req, res) => {
  ...
  try {
    ...
    if (venderEmail) {
      ...
      res.status(201).json({
        message: "Vender registerd successfully this msg is from backend",
        data: newVender,
      });
...
const venderLogin = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ venderId: vender._id }, secreatKey, {
      expiresIn: "5m",
    });
    ...
    res
      .status(201)
      .json({ message: "login sussfull", token, venderId, venderName });
  } catch (error) {
    console.error(error);
  }
};
